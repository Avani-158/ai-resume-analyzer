...
interface Candidate {
  name: string;
  score: number; // target role match
  ats: number;
  skills: string[];
  missing: number;
  composite: number;
  likelyResume: boolean;
  note?: string;
}
...
export default function EmployerPage() {
  ...
  const [sortBy, setSortBy]         = useState<"composite" | "score" | "ats">("composite");
  ...
  const handleFiles = (incoming: FileList | null) => {
    ...
    setFiles(prev => {
      ...
      if (fresh.length !== pdfs.length) {
        toast("Skipped duplicate files");
      }
    ...
    for (const file of files) {
      try {
        ...
        const hasCoreSections =
          Boolean(res.sections?.education) &&
          (Boolean(res.sections?.experience) || Boolean(res.sections?.projects));
        const likelyResume = hasCoreSections || res.skills.length >= 4;
        const composite = Number((res.target_match_score * 0.65 + res.ats_score * 0.35).toFixed(2));
        results.push({
          ...
          composite,
          likelyResume,
          note: likelyResume ? undefined : "This file may not be a resume (low profile signal)",
        });
      } catch {
  ...
  const toggle = (col: "composite" | "score" | "ats") => {
    if (sortBy === col) setSortDir(d => d === "desc" ? "asc" : "desc");
    else { setSortBy(col); setSortDir("desc"); }
  };

  const Icon = ({ col }: { col: "composite" | "score" | "ats" }) =>
    sortBy === col
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-surface-border text-slate-400 text-xs uppercase tracking-wider">
                    ...
                    <th className="px-6 py-3 text-left font-semibold cursor-pointer hover:text-white" onClick={() => toggle("composite")}>
                      <span className="flex items-center gap-1">Overall <Icon col="composite" /></span>
                    </th>
                  ...
                  {sorted.map((c, i) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        {c.note && <div className="text-xs text-yellow-400 mt-1">{c.note}</div>}
                      </td>
                      <td className="px-6 py-4">
                        <span className={`font-bold ${c.composite >= 70 ? "text-emerald-400" : c.composite >= 50 ? "text-yellow-400" : "text-red-400"}`}>
                          {c.composite}%
                        </span>
                      </td>
                      <td className="px-6 py-4">
